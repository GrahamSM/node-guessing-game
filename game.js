...
function checkBet(bet){
  if ((bet < 5) || (bet > 10)){
    console.log("Your bet must be between $5 and $10. Try again.");
    return false;
  }else {
    return true;
  }
...
function checkGuess(guess){
  if ((guess < 1) || (guess > 10)){
    console.log("Your guess must be between 1 and 10. Try again.")
    return false;
  }else {
    return true;
  }
...
function getOutcome(diff, bet, bankroll){
  console.log("Results from this round");
  if (diff===0){
    bankroll += +bet;
...
function getFinalStatus(bankroll){
  console.log("Your ending bankroll is: " + bankroll);
}
